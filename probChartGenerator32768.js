  ...
  for (j=0;j<round1["children"].length;j++){
    ...
    for (k=0;k<round2["children"].length;k++){
      ...
      for (l=0;l<round3["children"].length;l++){
        ...
        round4["name"]+=", probability = ["+(probabilities(round3["name"],round4["name"])).join(" × ") + "]"
      }
      round3["name"]+=", probability = ["+(probabilities(round2["name"],round3["name"])).join(" × ") + "]"
    }
    round2["name"]+=", probability = ["+(probabilities(round1["name"],round2["name"])).join(" × ") + "]"
  }
  round1["name"]+=", probability = ["+(probabilities(matchTree["name"],round1["name"])).join(" × ") + "]"
}
matchTree = [matchTree];
